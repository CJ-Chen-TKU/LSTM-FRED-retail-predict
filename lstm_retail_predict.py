...
# 1. Mount Google Drive (only needed when running in Colab)
...
• Plots recent actuals + multi‑step forecast with smooth continuity
"""
    ...
    if series_list:
        ...
        if len(data_filled.dropna()) < 24:
            ...
            if st.button("🚀 Train LSTM"):
                ...
                # Prepare actual data to plot: last 24 months including last training date
                last_train_date = df_train.index[-1]
                plot_start_date = last_train_date - pd.DateOffset(months=23)
                plot_actual = data_filled[target_col].loc[plot_start_date : last_train_date]

                # Forecast index
                forecast_idx = pd.date_range(start=last_train_date + pd.offsets.MonthBegin(), periods=horizon, freq="M")
                forecast_df = pd.DataFrame({target_col: inv_forecasts}, index=forecast_idx)

                st.metric("Next predicted value", f"{inv_forecasts[0]:,.2f}")

                # Plot actual + forecast with smooth continuity
                fig = px.line(title=f"{target_col}: Last 24 months and Forecast")
